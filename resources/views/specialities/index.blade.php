...
@section('content_header')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"> Специализации</li>
    </ol>
@stop
                    ...
                    <table class="table table-striped table-hover table-condensed">
                        ...
                        @foreach($specialities as $speciality)
                            <tr>
                                ...
                                <td class="text-left">{{ $speciality->name }}</td>
                                <td class="text-center"><span class="label label-default">{{ $speciality->active ? 'опубликовано' : 'скрыто от показа'}}</span></td>
                                <td class="text-center">
                                    <form class="" action="{{route('specialities.destroy',$speciality->id)}}" method="post">
                                        <input type="hidden" name="_method" value="delete">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <a href="{{route('specialities.edit',$speciality->id)}}" class="btn btn-info btn-xs">Редактировать</a>
